...
// Resolve returns the project id for r and the source it came from. Sources
// are tried in order; the chain exists because the forwarding shape Milo uses
// is not yet confirmed, and the losing sources get deleted once it is.
func Resolve(r *http.Request) (id string, source string, ok bool) {
	for _, candidate := range []struct {
		source string
		fn     func(*http.Request) string
	}{
		{"remote-extra", fromUserExtras},
		{"header", fromProjectHeader},
		{"path", fromPath},
	} {
